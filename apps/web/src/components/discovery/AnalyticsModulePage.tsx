...
export const AnalyticsModulePage = () => {
  const { module } = useParams<{ module: string }>();
  const analyticsModule = module ? getModuleByKey(module) : null;
  const { data, loading, error } = useAnalytics(module || null);

  if (!analyticsModule) {
    return (
      <EmptyState
        title="Module Not Found"
        description={`The analytics module "${module}" does not exist.`}
      />
    );
  }
